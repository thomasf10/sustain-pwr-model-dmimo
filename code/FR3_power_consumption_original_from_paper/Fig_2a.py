...
P_T = 100                                               # Total transmit power at base station [W]
...
eta_DPD = 2e12                                          # Computational efficiency of digital predistortion [GOPS/W]
...
Pbar_dig_load_ind = np.zeros(len(M_RF_vec))             # Load-independent part of digital's consumption [W]
Pbar_dig_load_dep = np.zeros(len(M_RF_vec))             # Load-dependent part of digital's consumption [W]
Pbar_ana_load_ind = np.zeros(len(M_RF_vec))             # Load-independent part of analog's consumption [W]
Pbar_ana_load_dep = np.zeros(len(M_RF_vec))             # Load-dependent part of analog's consumption [W]
Pbar_PA_load_ind = np.zeros(len(M_RF_vec))              # Load-independent part of power amplifier's consumption [W]
Pbar_PA_load_dep = np.zeros(len(M_RF_vec))              # Load-dependent part of power amplifier's consumption [W]


### Iterate over number of RF chains
for m_RF_index in range(len(M_RF_vec)):
    M_RF = M_RF_vec[m_RF_index]                         # Number of RF chains
    if M_RF == M_ant:
        M_PS = 0                                        # Number of phase shifters in fully-digital beamforming
    else:
        M_PS = M_ant/M_RF                               # Number of phase shifters in hybrid partially-connected beamforming
    ...
    P_enc = P_encoder_s + 1/eta_encoder*(f_sI*14/(3*8)*R_DL[m_RF_index]/B_tilde)
    P_dec = P_decoder_s + 1/eta_decoder*(f_sI*(5*35)/(2*3)*R_UL[m_RF_index]/B_tilde)
    ...
    Pbar_dig_load_ind[m_RF_index] = (Pbar_dig - xbar_DL*tau_DL*(1-tau_DLsig)*P_digDL+xbar_DL*tau_DL*(1-tau_DLsig)*P_digDL*delta_dig_micro -
                                     xbar_UL*tau_UL*(1-tau_ULsig)*P_digUL+xbar_UL*tau_UL*(1-tau_ULsig)*P_digUL*delta_dig_micro)
    Pbar_dig_load_dep[m_RF_index] = Pbar_dig - Pbar_dig_load_ind[m_RF_index]
    Pbar_dig_load_ind[m_RF_index] = (1/eta_dig_sc)*Pbar_dig_load_ind[m_RF_index]
    Pbar_dig_load_dep[m_RF_index] = (1/eta_dig_sc)*Pbar_dig_load_dep[m_RF_index]
    ...
    # Average over the different operating modes
    Pbar_anaUL = (xbar_UL*tau_UL*(1-tau_DLsig)*P_anaUL +
    ...
    # Split between load-independent and load-dependent term
    Pbar_ana_load_ind[m_RF_index] = (Pbar_ana - xbar_DL*tau_DL*(1-tau_DLsig)*P_anaDL+xbar_DL*tau_DL*(1-tau_DLsig)*P_anaDL*delta_ana_micro -
                                     xbar_UL*tau_UL*(1-tau_ULsig)*P_anaUL+xbar_UL*tau_UL*(1-tau_ULsig)*P_anaUL*delta_ana_micro)
    Pbar_ana_load_dep[m_RF_index] = Pbar_ana - Pbar_ana_load_ind[m_RF_index]
    Pbar_ana_load_ind[m_RF_index] = (1/eta_ana_sc)*Pbar_ana_load_ind[m_RF_index]
    Pbar_ana_load_dep[m_RF_index] = (1/eta_ana_sc)*Pbar_ana_load_dep[m_RF_index]
    ...
    # Split between load-independent and load-dependent term
    Pbar_PA_load_ind[m_RF_index] = (Pbar_PA - M_ant*(xbar_DL*tau_DL*(1-tau_DLsig)*P_PA(Pa,P_max,eta_PAmax)-
                                                xbar_DL*tau_DL*(1-tau_DLsig)*P_PA(0,P_max,eta_PAmax)*delta_PA_micro))
    Pbar_PA_load_dep[m_RF_index] = Pbar_PA - Pbar_PA_load_ind[m_RF_index]
    Pbar_PA_load_ind[m_RF_index] = (1/eta_PA_sc)*Pbar_PA_load_ind[m_RF_index]
    Pbar_PA_load_dep[m_RF_index] = (1/eta_PA_sc)*Pbar_PA_load_dep[m_RF_index]
...
for i in range(len(M_RF_vec)):
    xax = [i*3-.4,i*3+.4]
    yax_li = [Pbar_dig_load_ind[i], Pbar_ana_load_ind[i]]
    yax_ld = [Pbar_dig_load_dep[i], Pbar_ana_load_dep[i]]
    bar_colors_1 = [mcolors.to_rgba(c, alpha=0.9) for c in ['tab:green', 'tab:orange']]
    bar_colors_2 = [mcolors.to_rgba(c, alpha=0.4) for c in ['tab:green', 'tab:orange']]
    p1 = ax.bar(xax, yax_li, edgecolor='black', color=bar_colors_1, hatch=[None,'///'])
    p2 = ax.bar(xax, yax_ld, edgecolor='black', bottom=yax_li, color=bar_colors_2, hatch=[None,'///'])
...
Pbar_PA = Pbar_PA_load_ind+Pbar_PA_load_dep
Pbar_ana = Pbar_ana_load_ind+Pbar_ana_load_dep
Pbar_dig = Pbar_dig_load_ind+Pbar_dig_load_dep
Pcons = Pbar_PA+Pbar_ana+Pbar_dig
EE = ((R_UL*xbar_UL+R_DL*xbar_DL)/Pcons)*1e-6                  # Vector of energy efficiency [Mbit/J]
print('DL load =',np.round(xbar_DL,2),', UL load =',np.round(xbar_DL,2))
# print('EE = \{'+str(np.round(EE,2)))
print('M_RF = {' + ',\t '.join(str(x) for x in np.round(M_RF_vec)) + '}')
print('EE = {' + ',\t '.join(str(x) for x in np.round(EE,2)) + '} Mbit/J')
