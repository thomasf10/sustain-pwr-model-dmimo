...
supply. The distributed system of \Cref{sec:dmimo_dl_sysmodel} replaces that site by
...
Throughout, $l=1,\dots,L$ indexes APs, $k=1,\dots,K$ users, $q=1,\dots,Q$ subcarriers, and $i\in\{\mathrm{DL},\mathrm{UL}\}$ the link direction, matching \Cref{sec:dmimo_dl_sysmodel}. Hardware parameters carried over from \Cref{sec:pwr_model} keep their symbols and now refer to a single AP. The number of data subcarriers is written $q_{i}$ in \eqref{eq:rate} and $Q_{\max}$ in \eqref{eq:load_inst}; we use $Q$ for it throughout and reserve $q$ for the subcarrier index. \todo{align the subcarrier-count symbol across \Cref{sec:pwr_model} and \Cref{sec:dmimo_dl_sysmodel}.}
...
In the fully distributed operation of \Cref{sec:dmimo_dl_sysmodel} every AP serves
...
priced, since \Cref{sec:dmimo_dl_sysmodel} assumes perfect CSI \todo{add this}.}
\label{tab:split}
\ra{1.15}
\begin{tabular}{@{}llccc@{}}
\toprule
...
MIMO-processing terms $P_{\mathrm{MIMO},i,l}$ depend on the split.
...
\Cref{sec:dmimo_dl_sysmodel}, so S1 is charged for its fronthaul load but not
debited for the accuracy it loses. Channel estimation processing is likewise
unpriced: \Cref{sec:dmimo_dl_sysmodel} assumes perfect \gls{csi}, so neither the
...
\todo{Parameter values still to be sourced before the distributed model can be
evaluated. Where a value cannot be pinned down, report results as a sensitivity
range over it rather than picking a point value silently.

\emph{New parameters, with no counterpart in \Cref{sec:pwr_model} and therefore
no inherited value:}
\begin{itemize}
\item $\eta_{\mathrm{CPU,sc}}$ and $\eta_{\mathrm{FH,sc}}$ in \eqref{eq:pnet}:
      supply and cooling of the central unit (an inverse data-centre PUE) and of
      the fronthaul transceivers.
\item $\delta^{\mathrm{ds}}$, $P^{0}_{\mathrm{AP},l}$, $P^{0}_{\mathrm{FH},l}$ in
      \eqref{eq:pnet}: deep-sleep reduction factor and the AP and fronthaul base
      levels it scales. Only needed once $\mathcal{S}\neq\{1,\dots,L\}$.
\item $P_{\mathrm{sync}}$ in \eqref{eq:ana_avg_ap}: per-AP distribution of the
      frequency and phase reference, and reciprocity calibration.
\item $P_{\mathrm{CPU},0}$ in \eqref{eq:cpu}: always-on consumption of the
      central unit. The number of FPGAs it runs the centralized precoder on is
      also still implicit, since \eqref{eq:pmimo} names $\bar M_{\mathrm{RF}}$
      only for the AP side.
\item $P_{\mathrm{FH},0}$ and $\Pi_{\mathrm{FH}}$ in \eqref{eq:fh_basic}:
      currently borrowed from \cite{ngo2018total}; confirm or replace for the
      fronthaul technology assumed here.
\item $b_{\mathrm{FH}}$ in \eqref{eq:fh_s1}: fronthaul quantiser resolution. It
      sets the S1 fronthaul rate outright and is the largest single lever on it.
\item $\delta^{\mu}_{\mathrm{FH}}$ in \eqref{eq:fh_avg}: idle reduction factor of
      an optical transceiver, argued to sit near one but not sourced.
\item $C_{\mathrm{FH}}$ in \Cref{ssec:dmimo_ee}: fronthaul link capacity, needed
      before the S1 rates can be claimed feasible.
\end{itemize}

\emph{Inherited values fitted for a macro base station, which should be refitted
for AP-class hardware rather than reused:}
\begin{itemize}
\item $\xi$, $\alpha$, $\eta_{\mathrm{PA,max}}$ in \eqref{eq:pa}: small AP
      amplifiers, not macro-cell ones.
\item $P_{\max}$: fix the sizing convention of \Cref{rem:pa_sizing}, since
      $P_{\max}=\rho_{\max}/M$ and a fixed component rating give different
      scaling laws for the network static floor.
\item $\eta_{\mathrm{dig,sc}}$, $\eta_{\mathrm{ana,sc}}$,
      $\eta_{\mathrm{PA,sc}}$: a passively cooled outdoor AP enclosure is not a
      macro cabinet, and the cooling share of the overhead largely disappears.
\item $\delta^{\mu}_{c}$ and $\delta^{0}_{c}$: micro-sleep and idle reduction
      factors of AP-class hardware.
\item $P_{s}$ and the 32 chains per FPGA in \eqref{eq:pmimo}: \Cref{rem:fpga}
      shows this granularity dominates at small $M$, so check whether a
      four-antenna AP really carries a full FPGA at $P_{s}$.
\item $\eta_{\mathrm{pre}}$ and the other computational efficiencies: the AP-side
      and CPU-side processors are different device classes, so a single value
      shared by both nodes is unlikely to hold.
\item $P_{\mathrm{LO}}$ and $P_{\mathrm{ana,misc}}$: now paid once per AP rather
      than once per site, so the per-node value matters where the shared one did
      not.
\end{itemize}}
