\section{Distributed Massive MIMO Downlink System Model}
\label{sec:dmimo_dl_sysmodel}
...
\frac{\bar{\vect{w}}_{k}[q]}
{\sqrt{\sum_{q'=1}^{Q}\expt{\norm{\bar{\vect{w}}_{k}[q']}^{2}}}},
\end{equation}
...
downlink power allocated to that user across the OFDM block, so that
...
Throughout this work we assume \emph{perfect} \gls{csi}: every precoder is built
from the true channels, $\hat{\vect{h}}_{k}[q]=\vect{h}_{k}[q]$, so no channel
estimation is performed and there is no pilot contamination. The uplink-pilot and
TDD-reciprocity mechanism described below is therefore the practical way this
\gls{csi} would be acquired, not a source of estimation error in the present
model; modelling pilot-based estimation and its error covariance is left to
future work. The pilot overhead is still charged to the rate, through the frame
parameters of \Cref{sec:frame}: the pilots occupy the uplink signalling phase, of
duration $\tau_{\mathrm{UL}}\,\tau_{\mathrm{UL,sig}}$ relative to the frame, and
the downlink rate \eqref{eq:rate} carries the prelog
$\tau_{\mathrm{DL}}(1-\tau_{\mathrm{DL,sig}})$. The downlink-only evaluation used
here, in which the uplink phase carries nothing but pilots and the downlink phase
nothing but data, is the case $\tau_{\mathrm{UL,sig}}=1$ and
$\tau_{\mathrm{DL,sig}}=0$, so that the pilots take up a fraction
$\tau_{\mathrm{UL}}$ of the frame and the downlink prelog is
$\tau_{\mathrm{DL}}=1-\tau_{\mathrm{UL}}$.
...
centralized case is lost. The simplest choice is \gls{mr} transmission, which
points each beam along the local channel,
\begin{equation}
...
parametrized per user per AP: AP $l$ attaches a coefficient $\rho_{kl}\geq 0$ to
its beam for user $k$ and scales the block to radiate it across the OFDM block,
\begin{equation}
\vect{w}_{kl}[q]=\sqrt{\rho_{kl}}\,
\frac{\bar{\vect{w}}_{kl}[q]}
{\sqrt{\sum_{q'=1}^{Q}\expt{\norm{\bar{\vect{w}}_{kl}[q']}^{2}}}}.
\end{equation}
Only fractional power control is used: each AP splits its own budget
...
\subsection{Complexity of precoders}
\todo{compute complexity of the precoders so this can be used in the power model}