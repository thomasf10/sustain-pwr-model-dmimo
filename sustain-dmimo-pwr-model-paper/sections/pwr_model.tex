...
that connect them to the \gls{cpu}, and the \gls{cpu} itself,
\begin{equation}
\label{eq:pnet}
...
\end{align}
with $P_{\mathrm{CPU},0}$ the always-on consumption of the central unit. It does
not cover the baseband hardware: the \glspl{fpga} on which the collective array
is processed are already charged through the $\bar M P_{s}$ term of
\eqref{eq:pmimo}, with $\bar M=\lceil LM/32\rceil$, and the encoder and decoder
carry their own static power inside $P_{\mathrm{enc}}$ and $P_{\mathrm{dec}}$.
$P_{\mathrm{CPU},0}$ is therefore only what remains, the chassis and the
infrastructure of the central unit, together with the shared reference clock of
\Cref{sssec:sync}.
...
Under a data-sharing split (S2,S3) the same payload is duplicated to every \gls{ap}, so
...
the precoder. With $L=32$ and $\Pi_{\mathrm{FH}}=\SI{0.25}{\watt}$ per Gbit/s the
network cannot exceed $\SI{125}{\mega\bit\per\joule}$ however well everything
...
% \todo{source a value for $P_{\mathrm{sync}}$. The
...
The convention also fixes the operating budget, since $P_{\max}$ is a device
rating rather than a free parameter: at the \gls{fr3} amplifier of
\cite{peschiera2026fr3} the network budget is $P_{\mathrm{TX}}=LM\,P_{\max}$,
which is how \Cref{tab:params_sys} is set.
\end{remark}
...
$L$ \glspl{ap} with $M<32$ antennas each pays it $L$ times. At $L=32$ and $M=4$
the network runs $32$ FPGAs where a co-located array of the same $128$ antennas
runs $4$. Chopping a large array into many small ones leaves the per-chain
