...
\todo{will probably be redundant if we have the sys model}
Throughout, $l=1,\dots,L$ indexes the \glspl{ap}, $m=1,\dots,M$ the antennas of
one \gls{ap}, $k=1,\dots,K$ the single-antenna users, $q$ the subcarriers, and
$i\in\{\mathrm{DL},\mathrm{UL}\}$ the link direction. The \glspl{ap} are fully
digital, so each carries $M$ \gls{rf} chains, and the network holds $LM$
antennas in total. The co-located reference is one site of $M_{\mathrm{ant}}=LM$
antennas. Every \gls{ap} is assumed active and serves every user, and both
directions are evaluated at full physical resource load,
$\bar x_{\mathrm{DL}}=\bar x_{\mathrm{UL}}=1$, so all \glspl{ap} share one
network-wide operating load. \todo{align these symbols with
\Cref{sec:sysmodel} once the system model section is written; $\rho_{\max}$,
$P_{T,l}$, $R_{\mathrm{DL}}$, $R_{\mathrm{UL}}$ and the coherence block
$\tau_c=\tau_p+\tau_u+\tau_d$ are taken from there.}
...
where $\bar x_{i}\in[0,1]$ is the average physical resource load. The frame
...
powers $P_{T,l}=\sum_{q}\sum_{k}\expt{\norm{\vect{w}_{kl}[q]}^{2}}\leq\rho_{\max}$
supplied by the rate model, which the two operation modes fill in differently:
local operation meets the constraint with equality at every \gls{ap}, whereas
centralized operation rescales all \glspl{ap} by one common factor and brings
only the busiest of them to its budget. Assuming the \gls{ap} power is shared
equally across its antennas, $P_{a,l}=P_{T,l}/M$, and
\begin{equation}
\label{eq:pa_avg_ap}
...
\frac{\rho_{\max}}{\eta_{\mathrm{PA,max}}}\left[\xi + (1-\xi)\,u_{l}^{\alpha}\right],
\qquad u_{l} = \frac{P_{T,l}}{\rho_{\max}},
\end{equation}
so the network static floor is $L\xi\rho_{\max}/\eta_{\mathrm{PA,max}} =
...
twice under centralized operation: only a fraction $\bar u<1$ of the budget is
radiated, while the static floor is paid in full.
...
rate model of \Cref{sec:sysmodel} with the prelogs $\tau_{d}/\tau_{c}$ and
$\tau_{u}/\tau_{c}$ already applied, and the network energy efficiency is the
